<template>
  <div>
    <!-- row 1 -->
    <div class="tw-flex tw-flex-wrap tw--mx-3">
      <!-- card1 -->
      <div class="tw-w-full tw-max-w-full tw-px-3 tw-mb-6 sm:tw-w-1/2 sm:tw-flex-none xl:tw-mb-0 xl:tw-w-1/4">
        <div class="tw-relative tw-flex tw-flex-col tw-min-w-0 tw-break-words tw-bg-white tw-shadow-xl dark:bg-slate-850 dark:shadow-dark-xl tw-rounded-2xl tw-bg-clip-border">
          <div class="tw-flex-auto tw-p-4">
            <div class="tw-flex tw-flex-row tw--mx-3">
              <div class="tw-flex-none tw-w-2/3 tw-max-w-full tw-px-3">
                <div>
                  <p class="tw-mb-0 tw-font-sans tw-text-xs tw-font-semibold tw-leading-normal tw-uppercase dark:tw-text-white dark:tw-opacity-60">Total membres</p>
                  <h5 class="tw-mt-2 tw-mb-2 tw-text-lg tw-font-bold dark:tw-text-white">3,452</h5>

                </div>
              </div>
              <div class="tw-px-3 tw-text-right basis-1/3">
                <div class="tw-inline-block tw-w-12 tw-h-12 tw-text-center rounded-circle tw-bg-gradient-to-tl tw-from-blue-500 tw-to-violet-500">
                  <v-icon class="ni tw-leading-none ni-money-coins tw-text-lg tw-relative tw-top-2 tw-text-white" size="40">mdi-account-group</v-icon>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <!-- card2 -->
      <div class="tw-w-full tw-max-w-full tw-px-3 tw-mb-6 sm:tw-w-1/2 sm:tw-flex-none xl:tw-mb-0 xl:tw-w-1/4">
        <div class="tw-relative tw-flex tw-flex-col tw-min-w-0 tw-break-words tw-bg-white tw-shadow-xl dark:bg-slate-850 dark:shadow-dark-xl tw-rounded-2xl tw-bg-clip-border">
          <div class="tw-flex-auto tw-p-4">
            <div class="tw-flex tw-flex-row tw--mx-3">
              <div class="tw-flex-none tw-w-2/3 tw-max-w-full tw-px-3">
                <div>
                  <p class="tw-mb-0 tw-font-sans tw-text-xs tw-font-semibold tw-leading-normal tw-uppercase dark:tw-text-white dark:tw-opacity-60">Total recrutement</p>
                  <h5 class="tw-mt-2 tw-mb-2 tw-text-lg tw-font-bold dark:tw-text-white">2,300</h5>

                </div>
              </div>
              <div class="tw-px-3 tw-text-right basis-1/3">
                <div class="tw-inline-block tw-w-12 tw-h-12 tw-text-center rounded-circle tw-bg-gradient-to-tl tw-from-red-600 tw-to-orange-600">
                  <v-icon class="ni tw-leading-none ni-world tw-text-lg tw-relative tw-top-2 tw-text-white" size="40">mdi-podium-bronze</v-icon>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <!-- card3 -->
      <div class="tw-w-full tw-max-w-full tw-px-3 tw-mb-6 sm:tw-w-1/2 sm:tw-flex-none xl:tw-mb-0 xl:tw-w-1/4">
        <div class="tw-relative tw-flex tw-flex-col tw-min-w-0 tw-break-words tw-bg-white tw-shadow-xl dark:bg-slate-850 dark:shadow-dark-xl tw-rounded-2xl tw-bg-clip-border">
          <div class="tw-flex-auto tw-p-4">
            <div class="tw-flex tw-flex-row tw--mx-3">
              <div class="tw-flex-none tw-w-2/3 tw-max-w-full tw-px-3">
                <div>
                  <p class="tw-mb-0 tw-font-sans tw-text-xs tw-font-semibold tw-leading-normal tw-uppercase dark:tw-text-white dark:tw-opacity-60">Total spécialités</p>
                  <h5 class="tw-mt-2 tw-mb-2 tw-text-lg tw-font-bold dark:tw-text-white">265</h5>

                </div>
              </div>
              <div class="tw-px-3 tw-text-right basis-1/3">
                <div class="tw-inline-block tw-w-12 tw-h-12 tw-text-center rounded-circle tw-bg-gradient-to-tl tw-from-emerald-500 tw-to-teal-400">
                  <v-icon class="ni tw-leading-none ni-paper-diploma tw-text-lg tw-relative tw-top-3 tw-text-white" size="40">mdi-school</v-icon>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <!-- card4 -->
      <div class="tw-w-full tw-max-w-full tw-px-3 sm:tw-w-1/2 sm:tw-flex-none xl:tw-w-1/4">
        <div class="tw-relative tw-flex tw-flex-col tw-min-w-0 tw-break-words tw-bg-white tw-shadow-xl dark:bg-slate-850 dark:shadow-dark-xl tw-rounded-2xl tw-bg-clip-border">
          <div class="tw-flex-auto tw-p-4">
            <div class="tw-flex tw-flex-row tw--mx-3">
              <div class="tw-flex-none tw-w-2/3 tw-max-w-full tw-px-3">
                <div>
                  <p class="tw-mb-0 tw-font-sans tw-text-xs tw-font-semibold tw-leading-normal tw-uppercase dark:tw-text-white dark:tw-opacity-60">Total évenements</p>
                  <h5 class="tw-mt-2 tw-mb-2 tw-text-lg tw-font-bold dark:tw-text-white">430</h5>

                </div>
              </div>
              <div class="tw-px-3 tw-text-right basis-1/3">
                <div class="tw-inline-block tw-w-12 tw-h-12 tw-text-center rounded-circle tw-bg-gradient-to-tl tw-from-orange-500 tw-to-yellow-500">
                  <v-icon class="ni tw-leading-none ni-cart tw-text-lg tw-relative tw-top-3 tw-text-white" size="40">mdi-calendar-text</v-icon>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>

    <!-- cards row 2 -->
    <div class="tw-flex tw-flex-wrap tw-mt-6 tw--mx-3">
      <div class="tw-w-full tw-max-w-full tw-px-3 tw-mt-0 tw-mb-6 lg:tw-mb-0 lg:tw-w-6/12 lg:tw-flex-none">
        <div class="tw-relative tw-flex tw-flex-col tw-min-w-0 tw-break-words tw-bg-white tw-border-0 tw-border-solid tw-shadow-xl dark:bg-slate-850 dark:shadow-dark-xl dark:bg-gray-950 tw-border-black-125 tw-rounded-2xl tw-bg-clip-border">
          <div class="tw-p-4 tw-pb-0 tw-mb-0 rounded-t-4">
            <div class="tw-flex tw-justify-between">
              <h6 class="tw-mb-2 dark:tw-text-white">Sales by Country</h6>
            </div>
          </div>
          <div class="tw-overflow-x-auto">
            <table class="tw-items-center tw-w-full tw-mb-4 tw-align-top tw-border-collapse tw-border-gray-200 dark:border-white/40">
              <tbody>
              <tr>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-b w-3/10 tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-flex tw-items-center tw-px-2 tw-py-1">
                    <div>
                      <img src="@/assets/img/icons/flags/US.png" alt="Country flag" />
                    </div>
                    <div class="tw-ml-6">
                      <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Country:</p>
                      <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">United States</h6>
                    </div>
                  </div>
                </td>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-b tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Sales:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">2500</h6>
                  </div>
                </td>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-b tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Value:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">$230,900</h6>
                  </div>
                </td>
                <td class="tw-p-2 tw-text-sm tw-leading-normal tw-align-middle tw-bg-transparent tw-border-b tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-flex-1 tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Bounce:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">29.9%</h6>
                  </div>
                </td>
              </tr>
              <tr>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-b w-3/10 tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-flex tw-items-center tw-px-2 tw-py-1">
                    <div>
                      <img src="@/assets/img/icons/flags/DE.png" alt="Country flag" />
                    </div>
                    <div class="tw-ml-6">
                      <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Country:</p>
                      <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">Germany</h6>
                    </div>
                  </div>
                </td>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-b tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Sales:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">3.900</h6>
                  </div>
                </td>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-b tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Value:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">$440,000</h6>
                  </div>
                </td>
                <td class="tw-p-2 tw-text-sm tw-leading-normal tw-align-middle tw-bg-transparent tw-border-b tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-flex-1 tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Bounce:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">40.22%</h6>
                  </div>
                </td>
              </tr>
              <tr>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-b w-3/10 tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-flex tw-items-center tw-px-2 tw-py-1">
                    <div>
                      <img src="@/assets/img/icons/flags/GB.png" alt="Country flag" />
                    </div>
                    <div class="tw-ml-6">
                      <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Country:</p>
                      <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">Great Britain</h6>
                    </div>
                  </div>
                </td>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-b tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Sales:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">1.400</h6>
                  </div>
                </td>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-b tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Value:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">$190,700</h6>
                  </div>
                </td>
                <td class="tw-p-2 tw-text-sm tw-leading-normal tw-align-middle tw-bg-transparent tw-border-b tw-whitespace-nowrap dark:border-white/40">
                  <div class="tw-flex-1 tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Bounce:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">23.44%</h6>
                  </div>
                </td>
              </tr>
              <tr>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-0 w-3/10 tw-whitespace-nowrap">
                  <div class="tw-flex tw-items-center tw-px-2 tw-py-1">
                    <div>
                      <img src="@/assets/img/icons/flags/BR.png" alt="Country flag" />
                    </div>
                    <div class="tw-ml-6">
                      <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Country:</p>
                      <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">Brasil</h6>
                    </div>
                  </div>
                </td>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-0 tw-whitespace-nowrap">
                  <div class="tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Sales:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">562</h6>
                  </div>
                </td>
                <td class="tw-p-2 tw-align-middle tw-bg-transparent tw-border-0 tw-whitespace-nowrap">
                  <div class="tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Value:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">$143,960</h6>
                  </div>
                </td>
                <td class="tw-p-2 tw-text-sm tw-leading-normal tw-align-middle tw-bg-transparent tw-border-0 tw-whitespace-nowrap">
                  <div class="tw-flex-1 tw-text-center">
                    <p class="tw-mb-0 tw-text-xs tw-font-semibold tw-leading-tight dark:tw-text-white dark:tw-opacity-60">Bounce:</p>
                    <h6 class="tw-mb-0 tw-text-sm tw-leading-normal dark:tw-text-white">32.14%</h6>
                  </div>
                </td>
              </tr>
              </tbody>
            </table>
          </div>
        </div>
      </div>


      <div class="tw-w-full tw-max-w-full tw-px-3 tw-mt-0 lg:tw-w-6/12 lg:tw-flex-none">
        <div class="tw-border-black/12.5 tw-shadow-xl dark:bg-slate-850 dark:shadow-dark-xl tw-relative tw-flex tw-min-w-0 tw-flex-col tw-break-words tw-rounded-2xl tw-border-0 tw-border-solid tw-bg-white tw-bg-clip-border">
          <div class="tw-p-4 tw-pb-0 tw-mb-2 tw-rounded-t-4 tw-flex tw-justify-between tw-w-full">
            <h6 class="tw-mb-0 dark:tw-text-white">Liste des spécialités</h6>
            <button class="tw-bg-indigo-500 tw-text-white tw-text-xs tw-font-bold tw-px-3 tw-py-1 tw-rounded tw-outline-none focus:tw-outline-none tw-mr-1 tw-mb-1 tw-ease-linear tw-transition-all tw-duration-150" type="button">
              Voir tout
            </button>
          </div>
          <div class="tw-flex-auto tw-w-full">
            <div class="tw-relative tw-flex tw-flex-col tw-min-w-0 tw-break-words tw-bg-white tw-w-full tw-mb-2">
              <div class="tw-block tw-w-full tw-overflow-x-auto">
                <table class="tw-items-center tw-w-full tw-bg-transparent tw-border-collapse">
                  <thead class="thead-light">
                  <tr>
                    <th class="tw-px-6 tw-align-middle tw-pt-4 tw-pb-0 tw-text-xs tw-leading-tight tw-whitespace-nowrap tw-font-semibold tw-text-left">
                      Spécialités:
                    </th>
                    <th class="tw-px-6 tw-align-middle tw-pt-4 tw-pb-0 tw-text-xs tw-leading-tight tw-whitespace-nowrap tw-font-semibold tw-text-left">
                      Membres:
                    </th>
                    <th class="tw-px-6 tw-align-middle tw-pt-4 tw-pb-0 tw-text-xs tw-leading-tight tw-whitespace-nowrap tw-font-semibold tw-text-left tw-min-w-[200px]">
                      Pourcentage
                    </th>
                  </tr>
                  </thead>
                  <tbody>
                  <tr v-for="(fonction, index) in fonctions" v-if="index < 6" :key="index">
                    <th class="tw-border-t-0 tw-px-6 tw-align-middle tw-border-b tw-border-r-0 tw-text-xs tw-whitespace-nowra tw-p-4 tw-text-left">
                      {{ fonction?.nom }}
                    </th>
                    <td class="tw-border-t-0 tw-px-6 tw-align-middle tw-border-b tw-border-r-0 tw-text-xs tw-whitespace-nowrap tw-p-4">
                      45
                    </td>
                    <td class="tw-border-t-0 tw-px-6 tw-align-middle tw-border-b tw-border-r-0 tw-text-xs tw-whitespace-nowrap tw-p-4">
                      <div class="tw-flex tw-items-center">
                        <span class="tw-mr-2">60%</span>
                        <div class="tw-relative tw-w-full">
                          <div class="tw-overflow-hidden tw-h-2 tw-text-xs tw-flex tw-rounded tw-bg-red-200">
                            <div style="width: 60%" class="tw-shadow-none tw-flex tw-flex-col tw-text-center tw-whitespace-nowrap tw-text-white tw-justify-center tw-bg-red-500"></div>
                          </div>
                        </div>
                      </div>
                    </td>
                  </tr>
                  </tbody>
                </table>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  layout: "master",
  data() {
    return {

    };
  },
  methods: {

  },
  mounted(){

  }

};
</script>

<style>

</style>